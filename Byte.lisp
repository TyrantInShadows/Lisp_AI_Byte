...
(defun PocetneOpcije ()
    ...
    ;;(princ (LegitimnoPomeranjeNaPrazno 2 2 2 4))
...
(defun Pomeri (iDiska jDiska iPomeraj jPomeraj visina)
    ...
    (setf lista (ProveriIgraca iDiska jDiska visina))
    (if (> (length lista) 0)
        (if (LegitimnoPomeranje iDiska jDiska iPomeraj jPomeraj)
            (if (equalp (length x) 0)
                (if (equalp (LegitimnoPomeranjeNaPrazno iDiska jDiska iPomeraj jPomeraj) t)
                    (Pomeraj iDiska jDiska iPomeraj jPomeraj (- visina 1) lista)
                    '())
                (if (equalp (length lista) 1)
                    (Pomeraj iDiska jDiska iPomeraj jPomeraj (- visina 1) lista)
...
(defun ProveriPobedu (i j l)
    (setf lista (car (cdr (PronadjiElement iPomeraj jPomeraj (gameState-tabla state)))))
