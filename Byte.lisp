        ...
        (if (LegitimnoPomeranje iDiska jDiska iPomeraj jPomeraj)
            (if (equalp (length x) 0)
                (if (equalp (LegitimnoPomeranjeNaPrazno iDiska jDiska iPomeraj jPomeraj) t)
                    ...
                    (princ "greska u pomeranju na prazno polje"))
                (if (equalp (length lista12) 1)
                    (Pomeraj iDiska jDiska iPomeraj jPomeraj (- visina 1) lista12)
                    (if (equalp t (StackOnStack (car (cdr (PronadjiElement iDiska jDiska (gameState-tabla state)))) x 0))
                        (Pomeraj iDiska jDiska iPomeraj jPomeraj (- visina 1) lista12)
                        (princ "Greska u pomeranju sa steka na stek"))
                    )
                )
            (princ "Pomerate na neodgovarajuce polje")
        )
        (princ "Niste vi na potezu")
    )
    (napotezu)
)

(defun ProveriPobedu (i j l)
    (setf listaProveriPobedu (car (cdr (PronadjiElement i j (gameState-tabla state)))))
    (if (equalp (length listaProveriPobedu) (gameState-velicinaTable state))
        (if (equalp (PoslednjiDiskNaStack listaProveriPobedu 0) 'X)
            (AND (setf (gameState-rezultatIgre state)(cons (1+ (car (gameState-rezultatIgre state)))(cdr (gameState-rezultatIgre state))))
            (setf (gameState-tabla state)(PronadjiElement+ i j l 0 t "izbaci" 0))
            )
            (AND (setf (gameState-rezultatIgre state)(cons (car (gameState-rezultatIgre state))(list (1+ (car (cdr (gameState-rezultatIgre state)))))))
            (setf (gameState-tabla state)(PronadjiElement+ i j l 0 t "izbaci" 0))
            )
        ))
